...
class SweepPipeline:
    ...
    def _compute_remaining_to_capture(self, hand: list[Card]) -> int:
        ...
        if not self.memory.is_special_gone("leaf"):
            ...
            if leaf_control == 0:
                # Nemám garantovanú kontrolu (ani cez A/K) → horník nie je
                # zaistený, ráta sa do remaining. Platí ROVNAKO, či ho
                # držím ja bez ochrany, alebo súper — v oboch prípadoch
                # ho môžem stratiť v prospech A/K vonku.
    ...
    def _layer3(self, l2: Layer2Result, trick_number: int) -> Layer3Result:
        ...
        for suit in ("leaf", "acorn"):
            ...
            if self.memory.is_special_gone(suit):
                # Horník už padol — buď je bezpečne u mňa (Gate1 by inak
                # zablokoval sweep), riziko sa už nerieši.
                continue
    ...
    def _find_escape(self, candidate: CandidateResult,
        ...
        Počíta sa zo stavu ruky PO zahraní candidate.first_card — escape
        plán rieši čo mi zostane na ĎALŠIE štichy, takže karta ktorú
        práve vediem sa už do úvahy neberie (je preč). Vďaka tomu sa
        rôzni kandidáti (rôzne prvé karty) môžu reálne líšiť v tom,
        aký escape im ostáva.
        ...
        """
        # Ruka po zahraní first_card — základ pre všetky výpočty nižšie
        remaining_hand = [c for c in hand if c != candidate.first_card]

        # Spočítaj reálne extended wins cez všetky farby (zo zvyšku ruky)
        total_extended_wins = 0
        for suit in SUITS:
            my_cards = [c for c in remaining_hand if c.suit == suit]
            remaining = self.memory.remaining[suit]
            total_extended_wins += self._suit_extended_wins(my_cards, remaining)

        remaining_tricks = len(remaining_hand)

        if remaining_tricks == 0 or total_extended_wins >= remaining_tricks:
            # Plnú kontrolu (alebo first_card bola posledná karta) — escape nepotrebný
            return EscapeRoute(
